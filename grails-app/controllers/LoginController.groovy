...
class LoginController {
	...
	def openIdAuthenticate = {
		String openID = params['j_username']
	    	  println "LoginController: openidAuth"
		try {
	 ...
	 */
	def authfail = {
	    	  println "LoginController: authfail enter"
		...
		if (exception) {
			...
			if (exception instanceof DisabledException) {
			        println "LoginController: authfail disabled"
				msg = "[$username] is disabled."
			}
			else {
                 	    	  println "LoginController: authfail wrong username or password"
		...
		if (isAjax()) {
			...
			redirect action: auth, params: params
	    	  ...
	    	  println "LoginController: authfail exit"
	}
